		...
		
{
        ...
        if (now - last_heartbeat > 1000) {
            last_heartbeat = now;
            update((int)demuxer_get_percent_pos(mpctx->demuxer),(int)playing_audio_pts(mpctx->sh_audio, mpctx->d_audio, mpctx->audio_out),(int)demuxer_get_time_length(mpctx->demuxer),0);
	 char *inf = get_metadata(META_INFO_TITLE);
                if (!inf)
                    inf = strdup("");
	update_main_string(inf);
     }
 }
...
//    if(vo_config_count) mpctx->video_out->draw_osd();
...
#endif
     {
        ...
        if (now - last_heartbeat > 1000) {
            last_heartbeat = now;
            update((int)demuxer_get_percent_pos(mpctx->demuxer),(int)demuxer_get_current_time(mpctx->demuxer),(int)demuxer_get_time_length(mpctx->demuxer),0);
     }
    }
...
//============================ Handle PAUSE ===============================
            ...
            if (mpctx->osd_function == OSD_PAUSE) {
                mpctx->was_paused = 1;
                update((int)demuxer_get_percent_pos(mpctx->demuxer),(int)playing_audio_pts(mpctx->sh_audio, mpctx->d_audio, mpctx->audio_out),(int)demuxer_get_time_length(mpctx->demuxer),1);
                pause_loop();
            }
